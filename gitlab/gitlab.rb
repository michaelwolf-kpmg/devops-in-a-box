...
openldap:
  ...
  user_filter: 'memberOf=cn=devops-user,ou=roles,#{ENV['LDAP_BASE_DN']}'
  group_base: 'ou=roles,#{ENV['LDAP_BASE_DN']}'
  admin_group: 'cn=devops-admin'
  sync_ssh_keys: false
  attributes:
    username: 'uid'
    email: 'mail'
    ...
    last_name: 'sn'
eos
gitlab_rails['ldap_servers'] = YAML.load(ldapConfig)
